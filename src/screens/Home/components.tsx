...
export const WorkspaceCard = memo(function WorkspaceCard({
  ...
  workspace: Workspace;
  onDetails: PressHandler;
  onBook: PressHandler;
  premium?: boolean;
}) {
  const colors = useThemeColors();
  const availability = getAvailability(workspace);
  const amenities = workspace.amenities?.length
    ? workspace.amenities.slice(0, 4)
    : DEFAULT_AMENITIES;
  return (
    <View style={[styles.workspaceCardShadow, shadows.md]}>
      <View style={[styles.workspaceCard, { backgroundColor: colors.card }]}>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={`View details for ${workspace.name}`}
          onPress={() => onDetails(workspace)}
        >
          <Image
          ...
          <View style={styles.ratingBadge}>
            ...
            <Text style={styles.ratingText}>{getRating(workspace)}</Text>
          </View>
        </Pressable>
        <View style={styles.cardBody}>
          <Text
            style={[styles.workspaceName, { color: colors.foreground }]}
            numberOfLines={1}
          >
            {workspace.name}
          </Text>
          ...
          {premium && (
            ...
            >
              {
                PREMIUM_COPY[
                  Math.abs(String(workspace.id).length) % PREMIUM_COPY.length
                ]
              }
            </Text>
          )}
          ...
          <View style={styles.cardFooter}>
            <View>
              <Text
                style={[styles.fromText, { color: colors.mutedForeground }]}
              >
                {premium ? 'Starting from' : 'From'}
              </Text>
              <Text style={[styles.price, { color: colors.foreground }]}>
                PKR {formatPrice(workspace.price)}
                <Text
                  style={[styles.priceUnit, { color: colors.mutedForeground }]}
                >
                  /day
                </Text>
              </Text>
            ...
            <Pressable
              accessibilityRole="button"
              accessibilityLabel={`Book ${workspace.name}`}
              onPress={() => onBook(workspace)}
...
export const SkeletonCard = memo(function SkeletonCard() {
  ...
  return (
    <View style={[styles.skeleton, { backgroundColor: colors.card }]}>
      <View
        style={[styles.skeletonImage, { backgroundColor: colors.primaryMuted }]}
      />
      <View
        style={[
          styles.skeletonLine,
          { backgroundColor: colors.primaryMuted, width: '65%' },
        ]}
      />
      <View
        style={[
          styles.skeletonLine,
          { backgroundColor: colors.primaryMuted, width: '82%' },
        ]}
      />
    </View>
  );
...
export const EmptyState = memo(function EmptyState({
  ...
  const colors = useThemeColors();
  return (
    <View style={[styles.empty, { backgroundColor: colors.primaryMuted }]}>
...
function getRating(workspace: Workspace) {
  return (
    4.4 +
    (Math.abs(Number(workspace.id) || workspace.name.length) % 6) / 10
  ).toFixed(1);
}
function getAvailability(workspace: Workspace) {
  if (!workspace.available)
    return { label: 'Sold Out', color: '#DC2626', background: '#FEE2E2' };
  if (
    typeof workspace.availableCount === 'number' &&
    workspace.availableCount <= 2
  )
    return { label: 'Few Left', color: '#D97706', background: '#FEF3C7' };
...
const styles = StyleSheet.create({
  ...
  searchInput: { flex: 1, fontSize: 15, paddingVertical: 0, minHeight: 48 },
  filterButton: {
    width: 46,
    height: 46,
    marginRight: HOME_SPACING.xs,
    borderRadius: 13,
    alignItems: 'center',
    justifyContent: 'center',
    overflow: 'hidden',
  ...
  sectionHeader: {
    ...
    marginLeft: HOME_SPACING.md,
  },
  sectionTitle: { fontSize: 21, fontWeight: '800', letterSpacing: -0.4 },
  sectionSubtitle: { fontSize: 13, marginTop: HOME_SPACING.xxs },
  seeAll: { fontSize: 14, fontWeight: '800' },
  workspaceCardShadow: {
    width: 288,
    borderRadius: radii.md,
    marginRight: HOME_SPACING.md,
    backgroundColor: '#fff',
  },
  workspaceCard: { borderRadius: radii.md, overflow: 'hidden' },
  workspaceImage: { height: 166, width: '100%', backgroundColor: '#E6F4F1' },
  imageOverlay: {
    ...StyleSheet.absoluteFill,
    bottom: 'auto',
    height: 166,
    backgroundColor: 'rgba(4, 33, 31, 0.18)',
  },
  availabilityBadge: {
    position: 'absolute',
    top: HOME_SPACING.sm,
    left: HOME_SPACING.sm,
    flexDirection: 'row',
    alignItems: 'center',
    gap: 5,
    paddingHorizontal: 9,
  ...
  cardBody: { padding: HOME_SPACING.md },
  workspaceName: { fontSize: 18, fontWeight: '800', letterSpacing: -0.25 },
  metaRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  metaText: { fontSize: 12, fontWeight: '600', maxWidth: 80 },
  amenityRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: HOME_SPACING.xs,
  ...
  amenityText: { fontSize: 10, fontWeight: '800' },
  premiumDescription: {
    fontSize: 13,
    lineHeight: 18,
    marginTop: HOME_SPACING.sm,
  ...
  fromText: { fontSize: 11, fontWeight: '600' },
  price: { fontSize: 18, fontWeight: '900', marginTop: 1 },
  ...
  skeletonImage: { height: 166, borderRadius: 12 },
  skeletonLine: { height: 14, borderRadius: 7, marginTop: HOME_SPACING.md },
  empty: {
    alignItems: 'center',
    width: 288,
    borderRadius: radii.md,
  ...
  emptyTitle: { marginTop: HOME_SPACING.sm, fontSize: 16, fontWeight: '800' },
  emptyText: {
    marginTop: HOME_SPACING.xxs,
    fontSize: 13,
    textAlign: 'center',
